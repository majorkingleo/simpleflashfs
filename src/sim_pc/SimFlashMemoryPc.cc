...
{
	// AI generated by GitHub Copilot Claude Opus 4.7 START
	// Even read() touches the fstream cursor, so it must be locked
	// against concurrent write/erase/read calls. See the comment in
	// SimFlashMemoryPc.h for why this simulator-side lock exists.
	std::lock_guard<std::mutex> lock( m_mutex );
	// AI generated by GitHub Copilot Claude Opus 4.7 END
	file.seekg(address);
	file.clear();
