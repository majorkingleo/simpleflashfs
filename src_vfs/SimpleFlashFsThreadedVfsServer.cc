...
public:
    ...
    // Forward truncate to the wrapped handle while holding the mutex,
    // matching the locking discipline of every other call here.
    bool truncate( std::size_t new_size ) override {
        auto lock = std::scoped_lock(m_mutex);
        return m_handle->truncate( new_size );
    }
