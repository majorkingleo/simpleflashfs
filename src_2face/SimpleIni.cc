		...
		const std::string_view & key, const std::string_view & value, const std::string_view & comment )
{
	...
	for( auto o_next_key_pos = find_next_key(); o_next_key_pos;  o_next_key_pos = find_next_key() ) {
		...
		if(  size_to_overwrite > len_to_write ) {
			...
			// Shrink path. Previously this branch padded the difference
			// (size_to_overwrite - len_to_write) with trailing spaces
			// after the new line's '\n', which leaked across iterations
			// and accumulated up to 14 dangling bytes at EOF in test 46.
			// We now actually shrink the file: write the new (shorter)
			// content in place, shift the tail (anything after the old
			// key line) left by the shrink amount with a forward chunked
			// copy that uses a single stack-allocated buffer of
			// properties.line_buffer_size bytes (NO heap allocation,
			// same configurable budget that insert() uses), then call
			// truncate(). Falls back to in-line space padding when the
			// underlying FileInterface does not support truncate().
			const std::size_t shrink_amount = size_to_overwrite - len_to_write;

			// Fallback used when the backend can't truncate, or when
			// reading the tail fails. Writes the line WITHOUT the
			// trailing "\n", pads with spaces up to size_to_overwrite,
			// then emits "\n" so padding stays inside the line and
			// does not bleed past '\n'.
			auto pad_with_spaces = [&]() -> bool {
				file.seek( start );
				if( !sl.empty() ) {
				...
				for( unsigned i = 0; i < shrink_amount; ++i ) {
					if( !write( " " ) ) {
						return false;
					}
				}
				return write( std::string_view("\n") );
			};

			const std::size_t tail_start_old = start + size_to_overwrite;
			const std::size_t old_file_size  = file.file_size();
			const std::size_t tail_size      = old_file_size > tail_start_old
			                                 ? old_file_size - tail_start_old
			                                 : 0;
			const std::size_t new_file_size  = old_file_size - shrink_amount;

			// Write the new, shorter key line in place. Its end position
			// is start + len_to_write, exactly `shrink_amount` bytes
			...
			// Forward chunked shift of the tail: read chunk @ read_pos,
			// write it @ read_pos - shrink_amount. Forward order is safe
			// because each chunk's destination ends `shrink_amount`
			// bytes before the next chunk's source starts, so no source
			// byte is clobbered before it is read.
			const std::size_t BUFFER_SIZE = properties.line_buffer_size;
			char *shift_buffer = reinterpret_cast<char*>( alloca( BUFFER_SIZE ) );

			std::size_t bytes_remaining = tail_size;
			std::size_t read_pos        = tail_start_old;
			std::size_t write_pos       = tail_start_old - shrink_amount;

			while( bytes_remaining > 0 ) {
				const std::size_t chunk = std::min( bytes_remaining, BUFFER_SIZE );

				file.seek( read_pos );
				std::span<char> rbuf( shift_buffer, chunk );
				if( !file.read( rbuf ) ) {
					return pad_with_spaces();
				}
				...
				file.seek( write_pos );
				if( file.write( std::span<char>(shift_buffer, rbuf.size()) )
				    != rbuf.size() ) {
					return false;
				}
				...
				read_pos        += rbuf.size();
				write_pos       += rbuf.size();
				bytes_remaining -= rbuf.size();
			}

			// Drop the now-stale trailing bytes from the file.
			if( !file.truncate( new_file_size ) ) {
				return pad_with_spaces();
			}
